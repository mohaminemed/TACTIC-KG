...
LORA_DIR = config["agents"]["verifier"]["lora_dir"]
LORA_HYBRID_DIR = config["agents"]["verifier"]["lora_hybrid_dir"]

if isLoRa:
   TYPER_OUTPUT_PATH = f"outputs/unseen/ft_{BASE_MODEL}/typed_triplets.json"
   OUTPUT_PATH = f"outputs/unseen/ft_{BASE_MODEL}/verified_triplets.json"
else:
   TYPER_OUTPUT_PATH = f"outputs/unseen/{BASE_MODEL}/typed_triplets.json"
   OUTPUT_PATH = f"outputs/unseen/{BASE_MODEL}/verified_triplets.json"
...
def deduplicate_triplets(triplets):
    ...
    for t in triplets:
        ...
        key = (subj.strip().lower(), rel.strip().lower(), obj.strip().lower(),
               stype.lower(), otype.lower(), label.lower(), evidence.lower())
        if key not in seen:
            seen.add(key)
            deduped.append(t)
    return deduped
...
merge_model = SentenceTransformer(EMBEDDING_MERGE_MODEL, device=device)
...
def curator_agent(doc):
    triplets = doc.get("typed_triplets", [])
    text = doc.get("text", "")
    ...
    proposed_edges = []
    # LLM bridging for isolated subgraphs
    if t_merged:
        for trips in t_merged:
            prompt = build_verifier_prompt(text, trips) #, focus_triplets=t_low_conf)
            ...
            if generated_triplets != [] and isinstance(generated_triplets, list):
               proposed_edges.append(generated_triplets)
            else: 
               proposed_edges.append(trips)  # fallback to original if parsing fails
            print(f"[Verifier] Verified triplets: {generated_triplets}")

    # Flatten proposed edges
    proposed_flat = [t for sublist in proposed_edges for t in sublist] if proposed_edges else []


    return deduplicate_triplets(proposed_flat)

# -------------------- RUN VERIFIER ------------------------
results = []
start_time = time.time()
for i, doc in enumerate(all_test_docs[:NUM_DOCS]):
    source_id = doc.get("source_id", f"doc_{i}")
    print(f"\n🔍 Verifing document {i}...")
    print(f"Source ID: {source_id}")
    verified_triplets = curator_agent(doc)
    results.append({
        "id": doc["id"],
        "source_id": source_id,
        "text": doc["text"],
        "typed_triplets": doc["typed_triplets"],
        "verified_triplets": verified_triplets
    })
    print(f"Initial triplets: {len(doc['typed_triplets'])} | Verified: {len(verified_triplets)}")


end_time = time.time()
elapsed = end_time - start_time 
print(f"\n⏱️ Verification using fine-tuned model {BASE_MODEL} completed in {elapsed:.2f} seconds.")
# Save results
...
print(f"\n✅ Verification complete. Saved to {OUTPUT_PATH}")
